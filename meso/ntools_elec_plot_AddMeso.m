function ntools_elec_plot_AddMeso(varargin)
...
if isempty(char(elec_all{5}(:)))
    ...
    s = strncmpi('S',elec_all{5},1);
    m = strncmpi('M',elec_all{5},1);
end

elec_grid = elec_cell(g,:);
elec_depth = elec_cell(d,:);
elec_strip = elec_cell(s,:);
elec_meso = elec_cell(m,:);
elec_cell(or(g,d),:) = [];
...
elseif isequal(plt,5) || strcmpi(plt,'brain')
    showpart = 'Brain';
    nyu_plot(surf_brain,[],[],[]);
elseif strcmpi(plt,'GSM')
    showpart = 'GSM';
%     elec = cell2mat(elec_cell(:,2:4));
%     elec_name = char(elec_cell(:,1));
    nyu_plot(surf_brain,sph,cell2mat(elec_grid(:,2:4)),char(elec_grid(:,1)),'r',labelshow,aparc_annot); hold on; % Plot grid
    % Plot strip
    elec_name_s = char(elec_strip(:,1));
    elec_s = cell2mat(elec_strip(:,2:4));
    for i=1:size(elec_s,1)
        if isempty(aparc_annot)
            stripcolor = 'b';
        else % re-define strip color if plot with aparc
            stripcolor = 'w';
        end
        plotSpheres(elec_s(i,1),elec_s(i,2),elec_s(i,3),2,stripcolor);
        if labelshow==1
            [xx, yy, zz] = adjust_elec_label(elec_s(i,:)); % default radius = 2
            text('Position',[xx yy zz],'String',elec_name_s(i,:),'Color','w','VerticalAlignment','top');
        end
    end
    % Plot meso
    elec_name_m = char(elec_meso(:,1));
    elec_m = cell2mat(elec_meso(:,2:4));
    for i=1:size(elec_m,1)
        if isempty(aparc_annot)
            mesocolor = 'm';
        else % re-define strip color if plot with aparc
            mesocolor = 'w';
        end
        plotSpheres(elec_m(i,1),elec_m(i,2),elec_m(i,3),1,mesocolor);
        if labelshow==1
            [xx, yy, zz] = adjust_elec_label(elec_m(i,:)); % default radius = 2
            text('Position',[xx yy zz],'String',elec_name_m(i,:),'Color','w','VerticalAlignment','top');
        end
    end
    hold off;  
else
...
rect = [400, 300, 1250, 1250]; figure('OuterPosition',rect);
