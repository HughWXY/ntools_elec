function ntools_elec_plot_XJ(varargin)
...
elseif nargin>=2
    ...
    try fighdl = varargin{find(strcmp('fighdl',varargin))+1}; catch err; end    % Figure handle
    try elecnam = varargin{find(strcmp('elecnam',varargin))+1}; catch err; end  % Labels of the electrodes to be plotted
    try eleccol = varargin{find(strcmp('eleccol',varargin))+1}; catch err; end  % Colors of the electrodes to be plotted
    try elecconnct = varargin{find(strcmp('elecconnct',varargin))+1}; catch err; end % Plot a line between elec pairs showing coactivity?
...
if strcmpi(plt,'G') || strcmpi(plt,'S') || strcmpi(plt,'D') || strcmpi(plt,'GS')
...
%% Find the pre-selected electrodes in the MRI elec list

if strcmpi(plt,'selectelec')
    elecnam_idx = [];
    for en = 1:length(elecnam)
        mati = find(strcmpi(elecnam(en), elec_all{1}));
        if(~isempty(mati))
            elecnam_idx = [elecnam_idx, mati];
        else
            eleccol(en) = [];
            disp(sprintf('Non-matching electrode: %s', elecnam{en}));
        end
    end
    elecs_plot = elec_cell(elecnam_idx,:);
end
...
% The default is do not plot a line between elec pairs showing coactivity
if ~exist('elecconnct','var')
    elecconnct = 0;
end
...
elseif isequal(plt,5) || strcmpi(plt,'brain')
    showpart = 'Brain';
    nyu_plot(surf_brain,sph,[],[],[],[],[],[],0.6,plt,fighdl); % transparency alpha = 0.6
elseif isequal(plt,6) || strcmpi(plt,'selectelec')
    elec = cell2mat(elecs_plot(:,2:4));
    elec_name = char(elecs_plot(:,1));
    figure(fighdl);
    for i=1:size(elec,1)
        plotSpheres(elec(i,1),elec(i,2),elec(i,3),2,eleccol{i});
        if labelshow==1
            [xx, yy, zz] = adjust_elec_label(elec(i,:)); % default radius = 2
            text('Position',[xx yy zz],'String',elec_name(i,:),'Color','w','VerticalAlignment','top');
        end
    ...
    if exist('elecconnct','var')
        if elecconnct==1 && size(elecs_plot,1)==2
            hold on, plot3(elec(:,1), elec(:,2), elec(:,3), 'Color','k', 'LineWidth',2);
        end
    end
    hold off;
else
...
%% subfunctions 
%% nyu_plot
function nyu_plot(surf_brain,sph,elec,elecname,color,label,aparc_annot,radius,alpha,plt,fighdl)
